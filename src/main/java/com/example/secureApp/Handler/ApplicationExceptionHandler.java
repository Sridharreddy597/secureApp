...
public class ApplicationExceptionHandler {
	...
	protected ResponseEntity<Object> handleAccessDeniedException(AccessDeniedException ex, HttpServletRequest request) {
		...
		final HttpStatus status = HttpStatus.INTERNAL_SERVER_ERROR;
		logException(status, referencedId, request, ex);
		return new ResponseEntity<>("{\"referenceId\":" + referencedId + ",\"message\":\"Unauthorised Access \"}",
				status);
	}

	@ExceptionHandler(value = AuthenticationCredentialsNotFoundException.class)
	protected ResponseEntity<Object> handleAuthenticationCredentialsNotFound(AccessDeniedException ex,
